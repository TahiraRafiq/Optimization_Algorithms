% clc
% clear all
close all

load('Brain2D.mat');
FOV=256;
Nc = 12;
Nx =  FOV;
Ny =  FOV;
rate =8;


M=rate;
mask=zeros(Nx,Ny);
mask(1:M:end,:)=1;

%% Images of each coils

cim=ifftshift(ifft2(ifftshift(DATA)));


c_raw=DATA;

squared_img = power(abs(cim), 2);
sum_img = sum(squared_img, 3);
rsos = sqrt(sum_img);
figure,
imshow(abs(rsos),[])
%% different noise

min_a = min(min(c_raw(:)));
max_a = max(max(c_raw(:)));
for n=1:Nc
    norm(:,:,n) = (c_raw(:,:,n)-min_a)./abs(max_a-min_a); 
end 

%% Coil images
coil_image=ifftshift(ifft2(ifftshift(norm)));


figure(1),
for n=1:Nc
    subplot(2,ceil(Nc/2),n)
    imshow(abs(coil_image(:,:,n)),[])
end

%% 32x32 sample

Sample_from_k=zeros(40,40,Nc);
Sample_from_k(:,:,:)=norm(109:148,109:148,:);
 
%% Hamming window filteration
w = hann(40)*hann(40)';
FFT = fftshift(fft2(w)); % complex matrix
FFT_abs = abs(FFT); % absolut values of fft
imagesc(1+log10(FFT_abs)) % show fft results
w_new = ifft2(ifftshift(FFT)); % you need fft not fft_abs
figure,
imshow(abs(w_new),[])


% Dot multiplication of window with sample

for n=1:Nc
filtered_sample(:,:,n)=w_new.*Sample_from_k(:,:,n);
end

% fitting 32x32 in back and zero padding
padded_filter=zeros(Nx,Ny,Nc);
padded_filter(109:148,109:148,:)=filtered_sample(:,:,:);
 

%% Coil images for coil sensitivity from sample (filtered)
for n=1:Nc
    coil_image1(:,:,n)=ifftshift(ifft2(ifftshift(padded_filter(:,:,n))));   
end
figure(1),
for n=1:Nc
    subplot(2,ceil(Nc/2),n)
    imshow(abs(coil_image1(:,:,n)),[])
end

%% Combined SOS image (low resolution)for coil sensitivty
for n=1:Nc
squared_img(:,:,n) = power(abs(coil_image1(:,:,n)), 2);
end

sum_img = sum(squared_img, 3);
rsos1 = sqrt(sum_img);
figure,
imshow((abs(rsos1)),[])

%% Reference Image for error calculation and difference Image
for n=1:Nc
sq_img(:,:,n) = power(abs(coil_image(:,:,n)), 2);
end
s_img = sum(sq_img, 3);
xo = sqrt(s_img);
figure,
imshow((abs(xo)),[])

%% Coil sensitivity

for n=1:Nc
c_sens2(:,:,n)=coil_image1(:,:,n)./rsos1;
end
figure,
for n=1:Nc
    subplot(2,ceil(Nc/2),n)
    imshow(abs(c_sens2(:,:,n)),[])
end

norm = mask.*norm;
%% ifft noisy k space
image_domain_with_noise=zeros(Nx,Ny,Nc);
image_domain_with_noise = (ifftshift(ifft2(ifftshift(norm)))); 
figure
for n=1:Nc
    subplot(2,ceil(Nc/2),n)
    imshow(abs(image_domain_with_noise(:,:,n)),[])
end


conjugate_sens=conj(c_sens2);

sum_for_a=zeros(Nx,Ny,Nc);
for n=1:Nc
sum_for_a(:,:,n)=conjugate_sens(:,:,n).*image_domain_with_noise(:,:,n);
end
figure
for n=1:Nc
    subplot(2,ceil(Nc/2),n)
    imshow(abs(sum_for_a(:,:,n)),[])
end


para=0.0006;

reg=zeros(Nx,Ny,Nc);
for n=1:Nc
reg(:,:,n)=sum_for_a(:,:,n)+para*rsos1;
end


a=zeros(Nx,Ny);
    for n=1:Nc
        a=a+reg(:,:,n);
    end
figure,
imshow(abs(a),[])


b=zeros(Nx,Ny,101);
b(:,:,1)=a;
p=0;
r=zeros(Nx,Ny,101);
r(:,:,1)=a;
alpha=0.75;

for i=1:100
    add_reg=para*(eye(Nx,Ny))*p;
    for n=1:Nc
       Ewithp(:,:,n)=c_sens2(:,:,n).*b(:,:,i);
    end
       k_space=fftshift(fft2(fftshift(Ewithp)));
       k_sample=mask.*k_space;
          Eh_half=ifftshift(ifft2(ifftshift(k_sample)));
          Eh=conjugate_sens.*Eh_half;
        for n=1:Nc
          q(:,:,n)=Eh(:,:,n)+add_reg;
        end
              qsum=zeros(Nx,Ny);
                 for n=1:Nc
                   qsum=qsum+q(:,:,n);
                 end
                 p= qsum-a;
                        b(:,:,i+1)=b(:,:,i)-alpha.*p;  
 end

figure,
imshow(abs(b(:,:,21)),[])
j=[2:101];
delta=sum(sum(abs(b(:,:,j)-rsos)./(abs(rsos+eps))));
scal=zeros(1,100);
scal(:,:)=delta(:,:,:);
figure,
semilogy(j,scal)
hold on
semilogy(j,scala,'--*')
hold on
 semilogy(j,sca,'--^')
% legend('Gradient Descent fixed LRate','Conjugate Gradtient','Gradient Descent LRate=ELS')
 legend('Gradient Descent fixed LRate lamda=0.006','Gradient Descent fixed LRate','Conjugate Gradtient lamda=0.006')
 
error2 = (abs(xo)-abs(b(:,:,21))).^2;
RMSE2 = sqrt(sum(error2(:))/(Nx * Ny));
NRMSE2 = RMSE2/(Nx*Ny)


diff=abs(xo)-abs(b(:,:,10));
figure,
imshow(abs(diff),[])
imagesc(abs(diff))


